...
export function getLabelFromValue(
	...
	dropdownOptions: IDropdownStore[] | undefined,
) {
...
export function getLabelFromLat(
	latToFind: string | null,
	dropdownOptions: IDropdownCity[] | undefined,
) {
	if (dropdownOptions && latToFind) {
		const selectedOption = dropdownOptions.find(
			option => option.value.lat === latToFind,
		)
		return selectedOption ? selectedOption.label : null
	}
	return 'Velg sted'
